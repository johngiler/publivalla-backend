...
class ShoppingCenterViewSet(viewsets.ReadOnlyModelViewSet):
    """
    Listado público de centros activos. Detalle por slug (`/api/centers/mi-centro/`).

    Listado: filtros opcionales `search`, `catalog_status` (all|available|inactive),
    `location` (all|caracas|other). Paginación estándar (20 por página).
    """

    queryset = ShoppingCenter.objects.filter(is_active=True).order_by("slug")
    ...
    def get_queryset(self):
        qs = ShoppingCenter.objects.filter(is_active=True).order_by("slug")
        ...
        elif catalog_status in ("soon", "inactive"):
            qs = qs.filter(is_active=False)
        location = params.get("location", "all")
        if location == "caracas":
            qs = qs.filter(
                Q(name__icontains="caracas") | Q(city__icontains="caracas")
            )
        elif location == "other":
            caracas_q = Q(name__icontains="caracas") | Q(city__icontains="caracas")
